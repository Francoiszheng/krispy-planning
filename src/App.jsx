...
*/
function buildRestauSchedule(weekType, cfg) {
...
*/
function buildFTSchedule(services, friture, decoupe, ftParams) {
...
function RestauTab({ weekType, setWeekType, cfg, setCfg, weekDates, generated, onGenerate, lastGen }) {
...
function FTLaboTab({ services, setServices, weekDates, onGenerate, lastGen, ftSchedule, friture, decoupe, ftParams, setFtParams }) {
...
function WeekPreview({ weekType, label, dateRange, isActive, onClick }) {
  const team = TEAM_RESTAU.filter(p=>!p.meetingOnly);
  return (
    <button onClick={onClick} style={{
...
function DashboardJourJ({ restauPlan, ftPlan, weekType }) {
  const now = new Date();
...
export default function App() {
  ...
  const handleGenerate = useCallback(()=>{
    setRestauPlan(buildRestauSchedule(weekType, cfg));
    setFtPlan(buildFTSchedule(services, calcFriture(services), decoupe, ftParams));
    setLastGen(new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}));
  },[weekType, cfg, services, decoupe, ftParams]);
  ...
  return (
    <div style={{background:'#b8d5e0',minHeight:'100vh',color:'#1e293b',fontFamily:"'Montserrat','DM Sans','Segoe UI',system-ui,sans-serif"}}>
      ...
      <main style={{maxWidth:1500,margin:'0 auto',padding:'20px 24px'}}>
        ...
        {/* ── J-5 Lock Banner ── */}
        {tab!=='dashboard' && <LockBanner lockedDays={lockedDays} onUnlock={handleUnlock}/>}
