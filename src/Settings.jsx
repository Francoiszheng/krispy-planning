...
function TabEtablissements({ onRefresh }) {
  ...
  const [deletingEtab, setDeletingEtab] = useState(null);
  ...
  const fetch_ = useCallback(async()=>{
    const [etR,srvR,capR]=await Promise.all([supabase.from('etablissements').select('*').order('sort_order'),supabase.from('services').select('*, slots(*)').order('sort_order'),supabase.from('capacites').select('*').order('sort_order')]);
    setEtablissements(etR.data||[]); setServices((srvR.data||[]).map(s=>({...s,slots:(s.slots||[]).sort((a,b)=>a.sort_order-b.sort_order)}))); setCapacites(capR.data||[]); setLoading(false);
  },[]);
  useEffect(()=>{fetch_();},[fetch_]);

  const saveEtab=async f=>{const p={nom:f.nom,type:f.type,horaires_ouverture:f.horaires_ouverture,alternance_ab:f.alternance_ab,notes:f.notes,updated_at:new Date().toISOString()};if(f.id)await supabase.from('etablissements').update(p).eq('id',f.id);else await supabase.from('etablissements').insert({...p,sort_order:etablissements.length});setEditingEtab(null);fetch_();if(onRefresh)onRefresh();};
  ...
  return (
    <div>
      ...
      {etablissements.map(et=>{const isExp=expandedEtab===et.id;const etSrv=services.filter(s=>s.etablissement_id===et.id);return(
          ...
          <div style={{padding:'16px',cursor:'pointer'}} onClick={()=>setExpandedEtab(isExp?null:et.id)}>
            <div style={S.cardHeader}>
              <div>
                ...
                <div style={{marginTop:'4px'}}><span style={S.badge()}>{TYPES_ETABLISSEMENT.find(t=>t.value===et.type)?.label||et.type}</span>{et.alternance_ab&&<span style={S.badge(B.corail)}>A/B</span>}<span style={{fontSize:'12px',color:'#888',marginLeft:'4px'}}>{etSrv.length} service{etSrv.length>1?'s':''}</span></div>
              </div>
              <div style={{display:'flex',gap:'6px'}} onClick={e=>e.stopPropagation()}><button style={S.btnSecondary} onClick={()=>setEditingEtab(et)}>Modifier</button><button style={S.btnDanger} onClick={()=>setDeletingEtab(et)}>Suppr.</button></div>
            </div>
            {et.horaires_ouverture&&Object.keys(et.horaires_ouverture).length>0&&(<div style={{marginTop:'6px',fontSize:'12px',color:'#666',display:'flex',flexWrap:'wrap',gap:'8px'}}>{JOURS.map(j=>{const pl=et.horaires_ouverture[j.code];if(!pl?.length)return null;return <span key={j.code}><strong>{j.label}</strong> {pl.map((p,i)=><span key={i}>{i>0&&' + '}{p.debut}–{p.fin}</span>)}</span>;})}</div>)}
          </div>
          {isExp&&(<div style={{padding:'0 16px 16px',borderTop:`1px solid ${B.bleusto}`}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',margin:'14px 0 10px'}}><span style={{fontSize:'14px',fontWeight:'700',color:B.bluck}}>Services</span><button style={{...S.btnSmall,backgroundColor:B.bleusto,color:B.bluck,padding:'6px 14px'}} onClick={()=>setEditingService({_etabId:et.id})}>+ Service</button></div>
            {!etSrv.length&&<p style={{fontSize:'13px',color:'#999',fontStyle:'italic'}}>Aucun service.</p>}
            {etSrv.map(srv=>{const isSE=expandedService===srv.id;return(
              <div key={srv.id} style={{backgroundColor:B.bleustoLight,borderRadius:'8px',padding:'12px',marginBottom:'8px'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',cursor:'pointer'}} onClick={()=>setExpandedService(isSE?null:srv.id)}>
                  <div><span style={{fontWeight:'700',fontSize:'14px',color:B.bluck}}>{isSE?'▼':'▶'} {srv.nom}</span><div style={{marginTop:'3px'}}>{srv.jours.map(j=><span key={j} style={{...S.badge(B.white),fontSize:'10px',padding:'1px 6px'}}>{JOURS.find(d=>d.code===j)?.label||j}</span>)}<span style={{fontSize:'12px',color:'#555',marginLeft:'6px'}}>{srv.heure_debut}→{srv.heure_fin} · min {srv.effectif_min}</span></div></div>
                  <div style={{display:'flex',gap:'4px'}} onClick={e=>e.stopPropagation()}><button style={{...S.btnSmall,color:B.bluck,backgroundColor:B.white}} onClick={()=>setEditingService(srv)}>✎</button><button style={{...S.btnSmall,color:B.gochu,backgroundColor:'#fee'}} onClick={()=>setDeletingService(srv)}>✕</button></div>
                </div>
                {(srv.capacites_requises||[]).length>0&&<div style={{marginTop:'6px'}}><CapBadges ids={srv.capacites_requises}/></div>}
                {isSE&&(<div style={{marginTop:'10px',paddingTop:'10px',borderTop:`1px dashed ${B.bleustoDark}`}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'6px'}}><span style={{fontSize:'12px',fontWeight:'600',color:B.bluck}}>Slots ({srv.slots.length})</span><button style={{...S.btnSmall,backgroundColor:B.white,color:B.bluck,border:`1px solid ${B.bleustoDark}`}} onClick={()=>setEditingSlot({serviceId:srv.id,slot:null})}>+ Slot</button></div>
                  {!srv.slots.length&&<p style={{fontSize:'12px',color:'#999',fontStyle:'italic'}}>Aucun slot.</p>}
                  {srv.slots.map(slot=>(<div key={slot.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 10px',marginBottom:'3px',borderRadius:'6px',backgroundColor:slot.est_optionnel?'#fafafa':B.white}}>
                    <div><span style={{fontWeight:'600',fontSize:'12px'}}>{slot.nom}</span><span style={{fontSize:'11px',color:'#666',marginLeft:'6px'}}>{slot.heure_debut}→{slot.heure_fin} · {slot.nb_personnes} pers.</span>{slot.est_optionnel&&<span style={{...S.badge(B.corail),fontSize:'10px',marginLeft:'4px'}}>opt.</span>}<CapBadges ids={slot.capacites_requises}/></div>
                    <div style={{display:'flex',gap:'3px'}}><button style={{...S.btnSmall,color:B.bluck,backgroundColor:B.bleustoLight,fontSize:'11px'}} onClick={()=>setEditingSlot({serviceId:srv.id,slot})}>✎</button><button style={{...S.btnSmall,color:B.gochu,backgroundColor:'#fee',fontSize:'11px'}} onClick={()=>setDeletingSlot(slot)}>✕</button></div>
                  </div>))}
                </div>)}
              </div>);})}
          </div>)}
      ...
      {deletingEtab&&<ConfirmDelete message={`Supprimer « ${deletingEtab.nom} » et tous ses services/slots ?`} onConfirm={removeEtab} onCancel={()=>setDeletingEtab(null)}/>}
      {deletingService&&<ConfirmDelete message={`Supprimer « ${deletingService.nom} » ?`} onConfirm={removeService} onCancel={()=>setDeletingService(null)}/>}
          ...
          {JOURS.map(j=>{const plages=form.horaires_ouverture[j.code];const isO=!!plages;return(
            <div key={j.code} style={{padding:'8px',borderRadius:'8px',backgroundColor:isO?B.bleustoLight:'transparent',border:`1px solid ${isO?B.bleusto:'#eee'}`}}>
              ...
              {isO&&<div style={{marginTop:'6px',paddingLeft:'4px'}}>
                {plages.map((p,i)=><div key={i} style={{display:'flex',alignItems:'center',gap:'6px',marginBottom:'4px'}}><span style={{fontSize:'11px',color:'#888',width:'56px'}}>Plage {i+1}</span><input type="time" style={{...S.input,width:'110px',padding:'5px 8px',fontSize:'13px'}} value={p.debut} onChange={e=>updatePlage(j.code,i,'debut',e.target.value)}/><span style={{color:'#999',fontSize:'13px'}}>→</span><input type="time" style={{...S.input,width:'110px',padding:'5px 8px',fontSize:'13px'}} value={p.fin} onChange={e=>updatePlage(j.code,i,'fin',e.target.value)}/>{plages.length>1&&<button type="button" style={{...S.btnSmall,color:B.gochu,backgroundColor:'#fee'}} onClick={()=>removePlage(j.code,i)}>✕</button>}</div>)}
...
function SlotForm({initial,serviceId,capacites,onSave,onClose}) {
  ...
  return (
    <Modal title={initial?'Modifier le slot':'Nouveau slot'} onClose={onClose}>
      <div style={S.field}><label style={S.label}>Nom</label><input style={S.input} value={form.nom} onChange={e=>setForm({...form,nom:e.target.value})} placeholder="Ex: Mise en place…"/></div>
      <div style={S.row}>
    ...
    <div>
      ...
      {profils.map(prof=>{const caps=capacites.filter(c=>(prof.capacite_ids||[]).includes(c.id));return(
        <div key={prof.id} style={{...S.card,borderLeft:`4px solid ${B.corail}`}}>
          <div style={S.cardHeader}>
            <div><h4 style={{...S.cardTitle,fontSize:'15px'}}>{prof.nom}</h4>{prof.description&&<p style={{fontSize:'12px',color:'#666',margin:'2px 0 0'}}>{prof.description}</p>}</div>
    ...
    <Modal title={initial?`Modifier « ${initial.nom} »`:'Nouveau profil'} onClose={onClose}>
      ...
      <div style={S.field}><label style={{...S.label,fontSize:'14px',marginBottom:'10px'}}>Capacités</label>
        <div style={{display:'flex',flexDirection:'column',gap:'4px'}}>
          {capacites.map(cap=>{const checked=form.capacite_ids.includes(cap.id);return(
...
function TabEquipe({ onRefresh }) {
  ...
  const [editing,setEditing]=useState(null);
  const [deleting,setDeleting]=useState(null);
  const [showDispos,setShowDispos]=useState(null);

  const fetch_=useCallback(async()=>{
    const [eR,etR,pR]=await Promise.all([supabase.from('employees').select('*').order('sort_order'),supabase.from('etablissements').select('*').order('sort_order'),supabase.from('profils').select('*').order('sort_order')]);
    setEmployees(eR.data||[]);setEtablissements(etR.data||[]);setProfils(pR.data||[]);setLoading(false);
  },[]);
  useEffect(()=>{fetch_();},[fetch_]);

  const save=async form=>{
    const p={name:form.name,contract_hours:form.contract_hours,is_active:form.is_active,statut:form.statut,etablissement_id:form.etablissement_id||null,profil_id:form.profil_id||null};
    if(form.id){await supabase.from('employees').update(p).eq('id',form.id);}
    else{await supabase.from('employees').insert({...p,slug:form.name.toLowerCase().replace(/\s+/g,'-'),initials:form.name.split(' ').map(w=>w[0]||'').join('').toUpperCase().slice(0,3),color:'#b8d5e0',sort_order:employees.length}).select('id').single();}
    ...
    <div>
      ...
      {employees.map(emp=>{const etab=etablissements.find(e=>e.id===emp.etablissement_id);const prof=profils.find(p=>p.id===emp.profil_id);return(
        <div key={emp.id} style={{...S.card,opacity:emp.is_active?1:0.5,borderLeft:`4px solid ${B.bleusto}`}}>
          <div style={S.cardHeader}>
            <div>
              <h4 style={{...S.cardTitle,fontSize:'15px'}}>{emp.name}{!emp.is_active&&<span style={S.badge('#ccc')}>Inactif</span>}</h4>
...
function EmployeeForm({initial,etablissements,profils,onSave,onClose}) {
  const [form,setForm]=useState({id:initial?.id||null,name:initial?.name||'',contract_hours:initial?.contract_hours||39,is_active:initial?.is_active??true,statut:initial?.statut||'Salarié',etablissement_id:initial?.etablissement_id||'',profil_id:initial?.profil_id||''});
  return (
    <Modal title={initial?`Modifier ${initial.name}`:'Nouvel employé'} onClose={onClose}>
      <div style={S.field}><label style={S.label}>Nom</label><input style={S.input} value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Prénom Nom"/></div>
      <div style={{...S.field,padding:'12px',backgroundColor:B.bleustoLight,borderRadius:'10px',border:`1px solid ${B.bleusto}`}}>
        ...
        {!profils.length?<p style={{...S.infoText,color:B.gochu}}>Créez d'abord des profils dans l'onglet Profils.</p>:
          <select style={S.select} value={form.profil_id} onChange={e=>setForm({...form,profil_id:e.target.value})}>
      ...
      <div style={{display:'flex',gap:'10px',justifyContent:'flex-end',marginTop:'16px'}}><button style={S.btnSecondary} onClick={onClose}>Annuler</button><button style={S.btnPrimary} disabled={!form.name.trim()} onClick={()=>onSave(form)}>{initial?'Enregistrer':'Créer'}</button></div>
    </Modal>
  );
}
