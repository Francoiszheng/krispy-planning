...
function TabEtablissements({onRefresh}){
  ...
  const [expandedEtab,setExpandedEtab]=useState(null);const [expandedService,setExpandedService]=useState(null);
        ...
        <div style={{padding:'16px',cursor:'pointer'}} onClick={()=>setExpandedEtab(isExp?null:et.id)}>
          <div style={S.cardHeader}>
            <div>
              ...
              <div style={{marginTop:'4px'}}><span style={S.badge()}>{TYPES_ETABLISSEMENT.find(t=>t.value===et.type)?.label||et.type}</span><span style={{fontSize:'12px',color:'#888',marginLeft:'4px'}}>{etSrv.length} service{etSrv.length>1?'s':''}</span></div>
            </div>
...
function TabEquipe({onRefresh}){
  ...
  const save=async form=>{
    ...
    const p={name:fullName,contract_hours:form.contract_hours,is_active:form.is_active,statut:form.statut,etablissement_id:form.etablissement_id||null,profil_id:form.profil_id||null};
...
function EmployeeForm({initial,etablissements,profils,onSave,onClose}){
  // Split existing name into prénom/nom for editing
  ...
  return(
    <Modal title={initial?`Modifier ${initial.name}`:'Nouvel employé'} onClose={onClose}>
      <div style={S.row}>
        <div style={{...S.field,flex:1}}><label style={S.label}>Prénom</label><input style={S.input} value={form.prenom} onChange={e=>setForm({...form,prenom:e.target.value})} placeholder="Prénom"/></div>
        <div style={{...S.field,flex:1}}><label style={S.label}>Nom</label><input style={S.input} value={form.nom} onChange={e=>setForm({...form,nom:e.target.value})} placeholder="Nom"/></div>
      </div>
